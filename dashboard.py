...
#=================================== Filtrando para facilitar estudo
...
def display_status(date, location):
    ...
    recuperados_novos = "-" if df_data_on_date["Recuperadosnovos"].isna().values[0] else \
        f'{int(df_data_on_date["Recuperadosnovos"].values[0]):,}'.replace(",",".")
    casos_acumulados = "-" if df_data_on_date["casosAcumulado"].isna().values[0] else \
        f'{int(df_data_on_date["casosAcumulado"].values[0]):,}'.replace(",",".")
    acompanhamentos_novos = "-" if df_data_on_date["emAcompanhamentoNovos"].isna().values[0] else \
        f'{int(df_data_on_date["emAcompanhamentoNovos"].values[0]):,}'.replace(",", ".")
    casos_novos = "-" if df_data_on_date["casosNovos"].isna().values[0] else \
        f'{int(df_data_on_date["casosNovos"].values[0]):,}'.replace(",", ".")
    obitos_acumulado = "-" if df_data_on_date["obitosAcumulado"].isna().values[0] else \
        f'{int(df_data_on_date["obitosAcumulado"].values[0]):,}'.replace(",", ".")
    obitos_novos = "-" if df_data_on_date["obitosNovos"].isna().values[0] else \
        f'{int(df_data_on_date["obitosNovos"].values[0]):,}'.replace(",", ".")

    return(recuperados_novos,acompanhamentos_novos,
...
def update_map(date):
    ...
    fig = px.choropleth_mapbox(df_data_on_states, locations="estado", color="casosAcumulado",
                               center={"lat": -16.95, "lon": -47.78}, zoom=4,
                               geojson=brazil_states, color_continuous_scale="Redor",
                               opacity=0.55, hover_data={"casosAcumulado": True,
                                                        "casosNovos": True,
                                                        "obitosNovos": True,
                                                        "estado": True})
    fig.update_layout(
